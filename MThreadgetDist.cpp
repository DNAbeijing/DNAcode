// MThreadgetDist.cpp : 定义控制台应用程序的入口点。
//
...
int mycount = 3; //表示需要读取的文件的第 mycount 行
...
void memMap(SYSTEM_INFO &mysinf, __int64 &qwFileSize, __int64 &qwFileOffSet, HANDLE &hFileMapping, vector<vector<char>> &buf, vector<char> &tmp)
{
...
void myThread(vector<string> &files, vector<char> query, int M, double R, int st, double* &ans)
{
	int i = st;
	while (i < files.size())
	{
		ans[i] = (UCR(files[i], query, M, R));
		i = i + 4;
	}
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	mynum = 0;
	vector<char> tmp;
	memMap(mysinf, qwFileSize, qwFileOffSet, hFileMapping, buf, tmp);
	int tmpCount = 0;	//记录当前块的行数
	if (qwFileSize > 0)
	{
		mynum = mynum + buf.size() - 1;	//更新mynum至当前块的倒数第二行
		tmpCount = buf.size() - 1;
	}
	else
	{
		mynum = mynum + buf.size();	//说明已经映射到最后一块
		tmpCount = buf.size();
	}
	...
	{
		long start1 = clock();
		if (mycount > mynum)  //需要读取的行号比当前得到的块的最大行号大，则说明块数不够，需继续向下读取块
		{
			while (mycount > mynum)
			{
